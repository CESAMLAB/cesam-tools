//! Interface graphique (egui/eframe) du régulateur EtherNet/IP — page unique.
//!
...
use crate::actors::{EipServerMsg, SharedSnapshot, SharedStatus, SimulationMsg};
...
/// Application graphique du régulateur EtherNet/IP.
pub struct EipGui {
    sim: ActorRef<SimulationMsg>,
    net: ActorRef<EipServerMsg>,
    ...
    /// Liste blanche éditée en texte multiligne (un motif par ligne).
...
impl EipGui {
    ...
    fn apply_settings(&mut self, cfg: AppConfig) {
        ...
        let _ = self.net.cast(EipServerMsg::Reconfigure {
...
impl EipGui {
        ...
        egui::TopBottomPanel::top("entete").show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.add(egui::Label::new(
                    egui::RichText::new(format!("OREE — {}", t(Msg::AppSubtitle))).size(22.0).strong(),
                ))
                .on_hover_text("Regulation Unit over EtherNet/IP — CESAM-Lab");
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if let Ok(st) = self.status.lock() {
                        if st.listening {
                            let label = ui.colored_label(egui::Color32::from_rgb(0, 150, 0), format!("EtherNet/IP ● {}", st.addr));
                            if let Some(peer) = &st.peer {
                                label.on_hover_text(format!("client: {peer}"));
                            }
                        } else if let Some(err) = &st.error {
                            ui.colored_label(egui::Color32::from_rgb(200, 60, 60), format!("EtherNet/IP ✖ {err}"));
                        } else {
                            ui.colored_label(egui::Color32::GRAY, "EtherNet/IP …");
                        }
                    }
                });
            });
            ...
            if self.config.network.is_exposed() {
                ui.colored_label(egui::Color32::from_rgb(200, 140, 0), t(Msg::NetworkExposed));
            }
            ...
            .show(ctx, |ui| {
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    if ui
                        ...
                        .on_hover_text("EtherNet/IP : Setpoint")
                        .changed()
                    {
                    ...
                    ui.add_enabled_ui(!snap.auto, |ui| {
                        ...
                        if ui
                            ...
                            .on_hover_text("EtherNet/IP : ManualOutput (mode manuel)")
                            .changed()
                        {
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            Plot::new("tendance")
                ...
                .show(ui, |plot_ui| {
                    ...
                    plot_ui.line(Line::new(format!("{}   {:.1}", t(Msg::LegPv), snap.pv), pv).color(COLOR_PV));
                    plot_ui.line(Line::new(format!("{}   {:.0} %", t(Msg::LegOutput), snap.output), out).color(COLOR_OUT));
                });
        });
    }
    ...
    fn settings_window(&mut self, ctx: &egui::Context) {
        ...
        };
        {
            ...
            let allowlist_text = &mut self.allowlist_text;
            egui::Window::new(format!("⚙ {}", t(Msg::SettingsTitle)))
                ...
                .show(ctx, |ui| {
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ...
                        ui.label(egui::RichText::new(t(Msg::Network)).strong());
                        ...
                        ui.label(t(Msg::Allowlist));
                        ui.add(egui::TextEdit::multiline(allowlist_text).desired_rows(3).desired_width(f32::INFINITY));
        ...
        // Synchronise la liste blanche (texte multiligne → Vec<String>).
        let parsed: Vec<String> = self
            ...
            .filter(|l| !l.is_empty())
            .collect();
        self.settings_draft.network.allowlist = parsed;
