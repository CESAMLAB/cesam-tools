...
cargo run -p mock_bin_ru_modbus       # Startet den Regler
...
bleibt `mock_bin_ru_modbus`, aber die ausführbare Datei heißt **`orme`** — siehe
...
mock_bin_ru_modbus/   Regler-Binary
...
- Datei: `mock_ru_modbus.toml` im aktuellen Verzeichnis oder Pfad, der durch
...
`mock_ru_modbus.toml` bereitstellen. Lizenz **MIT** (Datei `LICENSE`).
...
Volume auf `/data` mounten, um `mock_ru_modbus.toml` bereitzustellen/zu persistieren.
