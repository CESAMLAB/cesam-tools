...
cargo run -p mock_bin_ru_modbus       # Run the controller
...
remains `mock_bin_ru_modbus`, but the executable is named **`orme`** — see
...
mock_bin_ru_modbus/   Controller binary
...
- File: `mock_ru_modbus.toml` in the current directory, or the path provided by
...
`mock_ru_modbus.toml`. License **MIT** (`LICENSE` file).
...
volume on `/data` to provide/persist `mock_ru_modbus.toml`.
