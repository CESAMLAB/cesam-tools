...
> **ORME** — *Open Regulator Modbus Emulator* · binary `mock_bin_ru_modbus` ·
...
### Saving settings
Settings are **saved** to a `mock_ru_modbus.toml` file (next to the software)
...
The folder mounted on `/data` allows you to provide/keep `mock_ru_modbus.toml`.
...
| My settings are not kept. | Click **Apply** / **💾 Save**. The `mock_ru_modbus.toml` file must be writable. |
