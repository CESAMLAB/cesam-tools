...
cargo run -p mock_bin_ru_modbus       # Lanza el regulador
...
sigue siendo `mock_bin_ru_modbus`, pero el ejecutable se llama **`orme`** — ver
...
mock_bin_ru_modbus/   Binario regulador
...
- Archivo: `mock_ru_modbus.toml` en el directorio actual, o ruta
...
raíz). Para distribuir: proporcionar el binario + un `mock_ru_modbus.toml`
...
`/data` para proporcionar/persistir `mock_ru_modbus.toml`.
