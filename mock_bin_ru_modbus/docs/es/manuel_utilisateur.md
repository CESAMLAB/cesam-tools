...
Los ajustes se **guardan** en un archivo `mock_ru_modbus.toml` (junto
...
La carpeta montada en `/data` permite proporcionar/conservar `mock_ru_modbus.toml`.
...
| Mis ajustes no se conservan. | Haga clic en **Aplicar** / **💾 Guardar**. El archivo `mock_ru_modbus.toml` debe ser accesible para escritura. |
