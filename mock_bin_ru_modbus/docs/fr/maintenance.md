...
cargo run -p mock_bin_ru_modbus       # Lance le régulateur
...
reste `mock_bin_ru_modbus`, mais l'exécutable s'appelle **`orme`** — voir
...
mock_bin_ru_modbus/   Binaire régulateur
...
- Fichier : `mock_ru_modbus.toml` dans le répertoire courant, ou chemin
...
racine). Pour distribuer : fournir le binaire + un `mock_ru_modbus.toml`
...
`/data` pour fournir/persister `mock_ru_modbus.toml`.
