...
> **ORME** — *Open Regulator Modbus Emulator* · binaire `mock_bin_ru_modbus` ·
...
Les réglages sont **sauvegardés** dans un fichier `mock_ru_modbus.toml` (à côté
...
Le dossier monté sur `/data` permet de fournir/conserver `mock_ru_modbus.toml`.
...
| Mes réglages ne sont pas conservés. | Cliquez **Appliquer** / **💾 Sauvegarder**. Le fichier `mock_ru_modbus.toml` doit être accessible en écriture. |
