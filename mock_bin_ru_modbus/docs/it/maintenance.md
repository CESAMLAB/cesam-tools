...
cargo run -p mock_bin_ru_modbus       # Lancia il regolatore
...
resta `mock_bin_ru_modbus`, ma l'eseguibile si chiama **`orme`** — vedi
...
mock_bin_ru_modbus/   Binario regolatore
...
- File: `mock_ru_modbus.toml` nella directory corrente, o percorso
...
radice). Per distribuire: fornire il binario + un `mock_ru_modbus.toml`
...
`/data` per fornire/persistere `mock_ru_modbus.toml`.
