...
> **ORME** — *Open Regulator Modbus Emulator* · binario `mock_bin_ru_modbus` ·
...
Le impostazioni sono **salvate** in un file `mock_ru_modbus.toml` (accanto
...
La cartella montata su `/data` permette di fornire/conservare `mock_ru_modbus.toml`.
...
| Le mie impostazioni non vengono conservate. | Cliccate **Applica** / **💾 Salva**. Il file `mock_ru_modbus.toml` deve essere accessibile in scrittura. |
