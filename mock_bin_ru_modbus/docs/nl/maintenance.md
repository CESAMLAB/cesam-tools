...
cargo run -p mock_bin_ru_modbus       # Start de regelaar
...
Cargo-pakket blijft `mock_bin_ru_modbus`, maar het uitvoerbare bestand heet
...
mock_bin_ru_modbus/   Regelaar-binair
...
- Bestand: `mock_ru_modbus.toml` in de huidige map, of het pad dat door de
...
`mock_ru_modbus.toml`. **MIT**-licentie (bestand `LICENSE`).
...
volume op `/data` om `mock_ru_modbus.toml` te leveren/behouden.
