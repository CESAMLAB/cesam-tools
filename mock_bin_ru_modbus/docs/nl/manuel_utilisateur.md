...
> **ORME** — *Open Regulator Modbus Emulator* · binair `mock_bin_ru_modbus` ·
...
De instellingen worden **opgeslagen** in een bestand `mock_ru_modbus.toml`
...
De map gemount op `/data` maakt het mogelijk `mock_ru_modbus.toml` te
leveren/behouden.
...
| Mijn instellingen worden niet bewaard. | Klik op **Toepassen** / **💾 Opslaan**. Het bestand `mock_ru_modbus.toml` moet schrijfbaar zijn. |
