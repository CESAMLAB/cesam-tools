...
cargo run -p mock_bin_ru_modbus       # Uruchamia regulator
...
pozostaje `mock_bin_ru_modbus`, ale plik wykonywalny nazywa się **`orme`** — zob.
...
mock_bin_ru_modbus/   Plik binarny regulatora
...
- Plik: `mock_ru_modbus.toml` w bieżącym katalogu lub ścieżka podana przez
...
`mock_ru_modbus.toml`. Licencja **MIT** (plik `LICENSE`).
...
`/data`, aby dostarczyć/utrwalić `mock_ru_modbus.toml`.
