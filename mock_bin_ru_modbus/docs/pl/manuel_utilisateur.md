...
> **ORME** — *Open Regulator Modbus Emulator* · plik binarny `mock_bin_ru_modbus` ·
...
### Zapis ustawień
Ustawienia są **zapisywane** w pliku `mock_ru_modbus.toml` (obok
...
Katalog zamontowany na `/data` pozwala dostarczyć/zachować `mock_ru_modbus.toml`.
...
| Moje ustawienia nie są zachowywane. | Kliknij **Zastosuj** / **💾 Zapisz**. Plik `mock_ru_modbus.toml` musi być dostępny do zapisu. |
