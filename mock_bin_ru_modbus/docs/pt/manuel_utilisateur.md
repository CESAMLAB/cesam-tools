...
> **ORME** — *Open Regulator Modbus Emulator* · binário `mock_bin_ru_modbus` ·
...
As regulações são **guardadas** num ficheiro `mock_ru_modbus.toml` (ao lado
...
A pasta montada em `/data` permite fornecer/conservar `mock_ru_modbus.toml`.
...
| As minhas regulações não são conservadas. | Clique **Aplicar** / **💾 Guardar**. O ficheiro `mock_ru_modbus.toml` deve estar acessível em escrita. |
