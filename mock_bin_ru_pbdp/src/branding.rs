...
/// Icône ORPD (régulateur PROFIBUS DP) — icône de fenêtre / barre des tâches.
/// Générée par `pic/ru_pbdp-logo.gen.py`.
pub const ORPD_ICON_PNG: &[u8] =
    include_bytes!(concat!(env!("CARGO_MANIFEST_DIR"), "/../pic/ru_pbdp-icon.png"));
...
/// Construit l'icône de fenêtre à partir de l'icône ORPD.
pub fn window_icon() -> Option<egui::IconData> {
    let image = image::load_from_memory(ORPD_ICON_PNG).ok()?.into_rgba8();
