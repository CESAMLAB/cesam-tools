//! Interface graphique (egui/eframe) du régulateur S7 — page unique.
//!
...
/// Application graphique du régulateur S7.
pub struct S7Gui {
    sim: ActorRef<SimulationMsg>,
    net: ActorRef<S7ServerMsg>,
...
impl S7Gui {
    ...
    fn apply_settings(&mut self, cfg: AppConfig) {
        ...
        let _ = self.net.cast(S7ServerMsg::Reconfigure {
        ...
        egui::TopBottomPanel::top("entete").show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.add(egui::Label::new(
                    egui::RichText::new(format!("ORSS — {}", t(Msg::AppSubtitle))).size(22.0).strong(),
                ))
                .on_hover_text("Regulation Unit over Siemens S7 — CESAM-Lab");
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    ...
                    if let Ok(st) = self.status.lock() {
                        if st.listening {
                            let label = ui.colored_label(egui::Color32::from_rgb(0, 150, 0), format!("S7 ● {}", st.addr));
                            if let Some(peer) = &st.peer {
                                label.on_hover_text(format!("client: {peer}"));
                            }
                        } else if let Some(err) = &st.error {
                            ui.colored_label(egui::Color32::from_rgb(200, 60, 60), format!("S7 ✖ {err}"));
                        } else {
                            ui.colored_label(egui::Color32::GRAY, "S7 …");
                        }
                    }
                });
            });
                ...
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    ui.add_enabled_ui(!snap.auto, |ui| {
                        ...
                        if ui
                            ...
                            .on_hover_text("S7 : DB1.DBD12 (mode manuel)")
                            .changed()
                        {
