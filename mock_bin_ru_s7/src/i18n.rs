...
//! acronymes (S7, DB, PID, %) restent codés en dur. Le compilateur garantit qu'aucune
...
impl Msg {
    ...
    fn entries(self) -> [&'static str; 8] {
        use Msg::*;
        match self {
            AppSubtitle    => ["Régulateur de procédé simulé (S7)", "Simulated process regulator (S7)", "Simulierter Prozessregler (S7)", "Regulador de proceso simulado (S7)", "Regolatore di processo simulato (S7)", "Regulador de processo simulado (S7)", "Gesimuleerde procesregelaar (S7)", "Symulowany regulator procesu (S7)"],
            ...
            NetworkExposed  => ["⚠ Serveur S7 exposé (toutes interfaces, liste blanche vide) — réseau de confiance uniquement", "⚠ S7 server exposed (all interfaces, empty allowlist) — trusted network only", "⚠ S7-Server offen (alle Schnittstellen, leere Whitelist) — nur vertrauenswürdiges Netzwerk", "⚠ Servidor S7 expuesto (todas las interfaces, lista blanca vacía) — solo red de confianza", "⚠ Server S7 esposto (tutte le interfacce, whitelist vuota) — solo rete attendibile", "⚠ Servidor S7 exposto (todas as interfaces, lista branca vazia) — apenas rede de confiança", "⚠ S7-server blootgesteld (alle interfaces, lege witte lijst) — alleen vertrouwd netwerk", "⚠ Serwer S7 odsłonięty (wszystkie interfejsy, pusta biała lista) — tylko zaufana sieć"],
            ...
            Network         => ["Réseau S7", "S7 network", "S7-Netzwerk", "Red S7", "Rete S7", "Rede S7", "S7-netwerk", "Sieć S7"],
