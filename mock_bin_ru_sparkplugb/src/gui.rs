//! Interface graphique (egui/eframe) du régulateur Sparkplug B — page unique.
//!
...
use crate::actors::{SharedSnapshot, SharedStatus, SimulationMsg, SparkplugMsg};
...
/// Application graphique du régulateur Sparkplug B.
pub struct SparkplugGui {
    sim: ActorRef<SimulationMsg>,
    net: ActorRef<SparkplugMsg>,
...
impl SparkplugGui {
    ...
    fn apply_settings(&mut self, cfg: AppConfig) {
        ...
        let _ = self.net.cast(SparkplugMsg::Reconfigure {
...
impl eframe::App for SparkplugGui {
        ...
        egui::TopBottomPanel::top("entete").show(ctx, |ui| {
            ...
            ui.horizontal(|ui| {
                ui.add(egui::Label::new(
                    egui::RichText::new(format!("ORSE — {}", t(Msg::AppSubtitle))).size(22.0).strong(),
                ))
                .on_hover_text("Regulation Unit over MQTT Sparkplug B — CESAM-Lab");
                    ...
                    if let Ok(st) = self.status.lock() {
                        if st.connected {
                            ui.colored_label(
                                ...
                                format!("Sparkplug B ● {} — {}", t(Msg::Connected), st.addr),
                            );
                        } else if let Some(err) = &st.error {
                            ui.colored_label(
                                egui::Color32::from_rgb(200, 60, 60),
                                format!("Sparkplug B ✖ {} — {err}", t(Msg::Disconnected)),
                            );
                        } else {
                            ui.colored_label(
                                egui::Color32::GRAY,
                                format!("Sparkplug B … {}", t(Msg::Disconnected)),
                            );
                        }
                    }
                });
            ...
            if self.config.network.tls {
                ui.colored_label(egui::Color32::from_rgb(0, 150, 0), t(Msg::SecurityTls));
            } else {
                ui.colored_label(egui::Color32::from_rgb(200, 140, 0), t(Msg::SecurityPlaintext));
            }
            ...
            .show(ctx, |ui| {
                egui::ScrollArea::vertical().show(ui, |ui| {
                    ...
                    if ui
                        ...
                        .on_hover_text("Sparkplug : Setpoint")
                        .changed()
                    {
                    ...
                    ui.add_enabled_ui(!snap.auto, |ui| {
                        ...
                        if ui
                            ...
                            .on_hover_text("Sparkplug : ManualOutput (mode manuel)")
                            .changed()
                        {
            ...
            egui::Window::new(format!("⚙ {}", t(Msg::SettingsTitle)))
                ...
                .show(ctx, |ui| {
                    egui::ScrollArea::vertical().show(ui, |ui| {
                        ...
                        ui.label(egui::RichText::new(t(Msg::Broker)).strong());
                        egui::Grid::new("net").num_columns(2).show(ui, |ui| {
                            ui.label(t(Msg::BrokerHost));
                            ui.text_edit_singleline(&mut draft.network.broker_host);
                            ui.end_row();
                            ui.label(t(Msg::BrokerPort));
                            ui.add(egui::DragValue::new(&mut draft.network.broker_port).range(1..=65535));
                            ui.end_row();
                            ui.label(t(Msg::ClientId));
                            ui.text_edit_singleline(&mut draft.network.client_id);
                            ui.end_row();
                            ui.label(t(Msg::GroupId));
                            ui.text_edit_singleline(&mut draft.network.group_id);
                            ui.end_row();
                            ui.label(t(Msg::EdgeNodeId));
                            ui.text_edit_singleline(&mut draft.network.edge_node_id);
                            ui.end_row();
                            ui.label(t(Msg::Keepalive));
                            ui.add(egui::DragValue::new(&mut draft.network.keepalive_secs).range(5..=3600));
                            ui.end_row();
                        });
                        ui.checkbox(&mut draft.network.tls, t(Msg::UseTls));
                        egui::Grid::new("auth").num_columns(2).show(ui, |ui| {
                            ui.label(t(Msg::Username));
                            ui.text_edit_singleline(&mut draft.network.username);
                            ui.end_row();
                            ui.label(t(Msg::Password));
                            ui.add(egui::TextEdit::singleline(&mut draft.network.password).password(true));
                            ui.end_row();
                        });
                        ui.checkbox(&mut draft.network.publish_on_change, t(Msg::PublishOnChange));
                        ui.add_enabled_ui(!draft.network.publish_on_change, |ui| {
                            egui::Grid::new("pub").num_columns(2).show(ui, |ui| {
                                ui.label(t(Msg::PublishPeriod));
                                ui.add(egui::DragValue::new(&mut draft.network.publish_period_secs).range(1..=3600));
                                ui.end_row();
                            });
                        });
