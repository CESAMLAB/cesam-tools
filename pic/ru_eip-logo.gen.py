#!/usr/bin/env python3
"""Génère le logo OREE (régulateur EtherNet/IP) — icône carrée + lockup horizontal.

Marque : **OREE** = *Open Regulator EtherNet/IP Emulator* (nom technique : RU/EtherNet/IP).
...
identité de régulateur), entouré d'un **anneau fermé de nœuds losanges** reliés tout
autour — un **Device-Level Ring (DLR)**, topologie redondante en anneau propre à
EtherNet/IP. L'anneau **fermé** (sans trou) et la forme **losange** des nœuds le
distinguent d'ORUE (anneau ouvert de nœuds ronds OPC UA) et d'ORSS (rack ouvert de
modules carrés S7).

Sorties :
  - ru_eip-icon.svg   (source vectorielle de l'icône carrée 256×256)
  - ru_eip-logo.svg   (lockup horizontal 760×240 : icône + texte)
  - ru_eip-icon.png   (rastérisation 256×256 via Pillow — asset embarqué + bureau)

Le SVG est la source de design ; le PNG est l'asset réellement embarqué
(`branding.rs`) et installé sur le bureau (`scripts/install-desktop.sh ru_eip`).
...
# Géométrie du cadran (réduit pour laisser place à l'anneau DLR autour).
R_ARC, R_FACE, ARC_W = 78, 62, 13
# Anneau fermé DLR : 8 nœuds losanges régulièrement répartis, reliés tout autour.
RING = 100
RING_ANGLES = [90, 135, 180, 225, 270, 315, 360, 45]
DS = 8.5  # demi-diagonale du losange (nœud)
...
def dial(cx, cy, scale=1.0):
    """Renvoie le <g> du cadran régulateur enveloppé de l'anneau DLR EtherNet/IP."""
    ...
    # Anneau DLR fermé : segments tout autour (incl. fermeture) + nœuds losanges.
    ...
    for nx, ny in ring_pts:
        dpts = " ".join(f"{f(x)},{f(y)}" for x, y in diamond(nx, ny, ds))
        el.append(f'<polygon points="{dpts}" fill="{ORANGE}"/>')
        dpts2 = " ".join(f"{f(x)},{f(y)}" for x, y in diamond(nx, ny, 3.4 * scale))
        el.append(f'<polygon points="{dpts2}" fill="{DARK}"/>')
    return "<g>\n    " + "\n    ".join(el) + "\n  </g>"


# --- Icône carrée 256×256 (cadran + anneau DLR centrés) ---
icon = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="256" height="256">
  <title>OREE — Open Regulator EtherNet/IP Emulator</title>
  {dial(128, 128)}
</svg>
'''
open("ru_eip-icon.svg", "w").write(icon)

# --- Lockup horizontal 760×240 (icône + texte) ---
# Marque **OREE**, même schéma qu'ORME/OSNE/ORUE : grand titre 4 lettres, deux
# dernières en orange (« OR » encre + « EE » orange). Le sous-titre est figé à une
# largeur fixe via `textLength` + `lengthAdjust` (police du visiteur variable).
FONT = "'DejaVu Sans','Segoe UI',Helvetica,Arial,sans-serif"
SUBTITLE_W = 420  # largeur fixe du sous-titre
logo = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 760 240" width="760" height="240">
  ...
  <text x="250" y="118" font-family="{FONT}" font-size="104" font-weight="800"
        class="ink" letter-spacing="2">OR<tspan fill="{ORANGE}">EE</tspan></text>
  <text x="252" y="158" font-family="{FONT}" font-size="27" font-weight="600"
        class="sub" letter-spacing="0.5"
        textLength="{SUBTITLE_W}" lengthAdjust="spacingAndGlyphs">Open Regulator EtherNet/IP Emulator</text>
  <text x="252" y="192" font-family="{FONT}" font-size="23" font-weight="700"
        fill="{ORANGE}" font-style="italic">« Connectez le procédé. »</text>
</svg>
'''
open("ru_eip-logo.svg", "w").write(logo)
...
def render_png(path, size=256, ss=4):
    ...
    # Anneau DLR fermé : segments tout autour + nœuds losanges.
    ring_pts = [pt(cx, cy, RING * sc, ang) for ang in RING_ANGLES]
    closed = ring_pts + [ring_pts[0]]
    for p1, p2 in zip(closed, closed[1:]):
        d.line([p1, p2], fill=orange + (255,), width=int(3.5 * sc))
    ds = DS * sc
    for nx, ny in ring_pts:
        d.polygon(diamond(nx, ny, ds), fill=orange + (255,))
        d.polygon(diamond(nx, ny, 3.4 * sc), fill=dark + (255,))

    img = img.resize((size, size), Image.LANCZOS)
    img.save(path)


render_png("ru_eip-icon.png")
print("écrit: ru_eip-icon.svg, ru_eip-logo.svg, ru_eip-icon.png")
