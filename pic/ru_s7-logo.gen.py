#!/usr/bin/env python3
"""Génère le logo ORSS (régulateur S7) — icône carrée + lockup horizontal.

Marque : **ORSS** = *Open Regulator S7 Server* (nom technique : RU/S7 ; le double
« S » final se lit « S-Seven » → S7, marque prononçable cohérente avec ORME/OSNE/ORUE).
...
identité de régulateur), entouré d'un **rack de modules** — des **nœuds carrés**
(modules d'automate / blocs de données) alignés sur un fond de panier (chaîne ouverte,
trou en bas comme l'ouverture du cadran). Évoque l'**automate Siemens S7** et son rack.
C'est ce qui distingue cet instrument d'ORME (bus Modbus), d'OSNE (agitateur), d'ORUE
(nœuds ronds OPC UA) et d'OREE (anneau fermé EtherNet/IP).

Sorties :
  - ru_s7-icon.svg   (source vectorielle de l'icône carrée 256×256)
  - ru_s7-logo.svg   (lockup horizontal 760×240 : icône + texte)
  - ru_s7-icon.png   (rastérisation 256×256 via Pillow — asset embarqué + bureau)

Le SVG est la source de design ; le PNG est l'asset réellement embarqué
(`branding.rs`) et installé sur le bureau (`scripts/install-desktop.sh ru_s7`).
...
# Géométrie du cadran (réduit pour laisser place au rack de modules autour).
R_ARC, R_FACE, ARC_W = 78, 62, 13
# Rack de modules S7 : nœuds carrés reliés par le fond de panier (chaîne ouverte,
# trou en bas comme l'ouverture du cadran).
RING = 100
RING_ANGLES = [135, 171, 207, 243, 279, 315, 351, 27]
HS = 7  # demi-côté du carré (module)
...
def dial(cx, cy, scale=1.0):
    """Renvoie le <g> du cadran régulateur enveloppé du rack de modules S7."""
    ...
    # Rack S7 : fond de panier (chaîne ouverte) + modules carrés.
    ...
    for nx, ny in ring_pts:
        el.append(f'<rect x="{f(nx-hs)}" y="{f(ny-hs)}" width="{f(2*hs)}" '
                  f'height="{f(2*hs)}" rx="{f(1.5*scale)}" fill="{ORANGE}"/>')
        el.append(f'<rect x="{f(nx-2.8*scale)}" y="{f(ny-2.8*scale)}" '
                  f'width="{f(5.6*scale)}" height="{f(5.6*scale)}" fill="{DARK}"/>')
    return "<g>\n    " + "\n    ".join(el) + "\n  </g>"


# --- Icône carrée 256×256 (cadran + rack centrés) ---
icon = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 256 256" width="256" height="256">
  <title>ORSS — Open Regulator S7 Server</title>
  {dial(128, 128)}
</svg>
'''
open("ru_s7-icon.svg", "w").write(icon)

# --- Lockup horizontal 760×240 (icône + texte) ---
# Marque **ORSS**, même schéma qu'ORME/OSNE/ORUE : grand titre 4 lettres, deux
# dernières en orange (« OR » encre + « SS » orange). Le sous-titre est figé à une
# largeur fixe via `textLength` + `lengthAdjust` (police du visiteur variable).
FONT = "'DejaVu Sans','Segoe UI',Helvetica,Arial,sans-serif"
SUBTITLE_W = 340  # largeur fixe du sous-titre (texte plus court)
logo = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 760 240" width="760" height="240">
  ...
  <text x="250" y="118" font-family="{FONT}" font-size="104" font-weight="800"
        class="ink" letter-spacing="2">OR<tspan fill="{ORANGE}">SS</tspan></text>
  <text x="252" y="158" font-family="{FONT}" font-size="27" font-weight="600"
        class="sub" letter-spacing="0.5"
        textLength="{SUBTITLE_W}" lengthAdjust="spacingAndGlyphs">Open Regulator S7 Server</text>
  <text x="252" y="192" font-family="{FONT}" font-size="23" font-weight="700"
        fill="{ORANGE}" font-style="italic">« Automatisez le procédé. »</text>
</svg>
'''
open("ru_s7-logo.svg", "w").write(logo)
...
def render_png(path, size=256, ss=4):
    ...
    hs = HS * sc
    for nx, ny in ring_pts:
        d.rectangle([nx - hs, ny - hs, nx + hs, ny + hs], fill=orange + (255,))
        d.rectangle([nx - 2.8 * sc, ny - 2.8 * sc, nx + 2.8 * sc, ny + 2.8 * sc], fill=dark + (255,))

    img = img.resize((size, size), Image.LANCZOS)
    img.save(path)


render_png("ru_s7-icon.png")
print("écrit: ru_s7-icon.svg, ru_s7-logo.svg, ru_s7-icon.png")
